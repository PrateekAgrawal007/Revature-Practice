PART 1: JavaScript Practice Exercises (Questions & Answers)

This document contains all 15 practice exercise questions with their answers.
...
Answer:
The execution order is Start → End → Async Task.
Synchronous code executes first in the Call Stack. The setTimeout callback is sent to the Callback Queue and is executed only after the Call Stack becomes empty with the help of the Event Loop.
...
Answer:
Even with 0ms, setTimeout waits until the Call Stack is empty. The callback is placed in the Callback Queue and executed later by the Event Loop.

3. JSON.parse()

Question:
You receive the following API response as a string:
{"id":101,"name":"Laptop","price":50000}
Convert it into a JavaScript object and access the name.

Answer:

let data = '{"id":101,"name":"Laptop","price":50000}';
...
Question:
You have a list of users and want to print a welcome message for each user using forEach().

Answer:

let users = ["Prateek", "Amit", "Ravi"];
...
Question:
An e-commerce site gives a 10% discount on all prices.
...
let availableProducts = products.filter(p => p.inStock === true);
...
Question:
Create a function to calculate GST using function declaration and arrow function.
...
Question:
A payment flow involves validating a user, processing payment, and generating an invoice.
Explain why nested callbacks become difficult to manage.

Answer:
Nested callbacks reduce readability, increase complexity, and make debugging difficult.
This problem is known as Callback Hell.

13. Promises & async/await

Question:
Convert the following Promise-based API call into async/await:
...
Question:
You click a button and dynamically add a new list item to a <ul>.
